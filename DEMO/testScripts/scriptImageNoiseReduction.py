...
""" Function: slice_HaarPSI
    Description:
        * This function computes the HaarPSI
    Inputs:
        * sc: input image
        * sc_ref: reference image
        * vmin, vmax: Minimum and maxmimum values of the image
    Outputs:
        * None
"""
def slice_HaarPSI(sc, sc_ref, vmax, vmin):
    sc_ref2 = 255*(np.clip(sc_ref, vmin, vmax) - vmin)/(vmax-vmin)
    sc2 = 255*(np.clip(sc, vmin, vmax) - vmin)/(vmax-vmin)
    sim, _, _ = haarPsi.haar_psi_numpy(sc_ref2, sc2 )
    return( sim )
...
"""
def evalCNN(qdct, cnn, isLWFSN=False, bypass_shrinkage=False):
    p=128
    ...
    with torch.no_grad():
        ...
        if isLWFSN:
            eva = cnn( inpp, bypass_shrinkage=bypass_shrinkage )
        else:
            eva = cnn( inpp )
...
def displaySlice(sli, ref, ax, xc, yc, of, zoom, vmin, vmax, meas=True, isRef= False):
    ...
    if meas:
        ax.text(180, 430, "HaarPsi=%1.2f [AU]"%(
            slice_HaarPSI(sli[yc:yc+of,xc:xc+of], ref[yc:yc+of,xc:xc+of], vmax, vmin)),
                fontsize=fs, color='red', )
        ax.text(180, 465, "MSSIM=%1.2f [AU]"%(
            slice_ssim(sli[yc:yc+of,xc:xc+of], ref[yc:yc+of,xc:xc+of])),
                fontsize=fs, color='red', )
        ax.text(180, 500, "PSNR=%1.1f [dB]"%(
...
def figureNoiseReductionCNNComparison(dataDir, dhsn2, dhsn1, fbp, red):
    ...
    # Making the figure
    sc=5
    fig, axs = plt.subplots(nrows=4, ncols=6,figsize=(sc*6,sc*4.11))

    
    dispDat = {
        0:{"Name":"N100", "xc":140, "yc":300, "of": 80,
           "axs":[axs[0,0], axs[0,1], axs[0,2], axs[0,3], axs[0,4], axs[0,5]]},
        1:{"Name":"N079", "xc":250, "yc":200, "of": 80,
           "axs":[axs[1,0], axs[1,1], axs[1,2], axs[1,3], axs[1,4], axs[1,5]]},
        2:{"Name":"N181", "xc":190, "yc":60, "of": 80,
           "axs":[axs[2,0], axs[2,1], axs[2,2], axs[2,3], axs[2,4], axs[2,5]]},
        3:{"Name":"N300", "xc":170, "yc":200, "of": 80,
    ...
    for i in np.arange(4):
        ...
        if i==0:
            ...
            dispDat[i]["axs"][1].set_title("LWFSN", fontsize= fs)
            dispDat[i]["axs"][2].set_title("rLWFSN", fontsize= fs)
            dispDat[i]["axs"][3].set_title("TF U-Net", fontsize= fs)
            dispDat[i]["axs"][4].set_title("FPBConvNet", fontsize= fs)
...
def reconLWFSN(dataDir, test_scan, cnn, xc, yc, of, level, width):
                
    scale=5
    fig, axs = plt.subplots(nrows=1, ncols=2, figsize=(scale*1.9456,scale*2))
    ...
    with h5py.File(dataDir, 'r') as f:
        inp = f["%s_qdct"%( test_scan )][:]

    res = evalCNN(inp, cnn, True, True)
    ...
    axs[0].add_patch(patches.Rectangle((xc,yc),of,of,linewidth=2,edgecolor='c',facecolor='none'))
    inset1 = zoomed_inset_axes(axs[0], zoom=zoom, loc='lower left')
    img = inset1.imshow(inp[yc:yc+of,xc:xc+of],cmap="gray",vmin=a_min, vmax=a_max)
    inset1.yaxis.set_major_locator(plt.NullLocator())
    inset1.xaxis.set_major_locator(plt.NullLocator())
    inset1.patch.set_edgecolor('cyan')
    inset1.patch.set_linewidth('10') 
    ...
    inset1 = zoomed_inset_axes(axs[1], zoom=zoom, loc='lower left')
    img = inset1.imshow(res[yc:yc+of,xc:xc+of],cmap="gray",vmin=a_min, vmax=a_max)
    inset1.yaxis.set_major_locator(plt.NullLocator())
    inset1.xaxis.set_major_locator(plt.NullLocator())
    dif = np.abs(res-inp)
    inset2 = zoomed_inset_axes(axs[1], zoom=zoom, loc='upper left')
    img = inset2.imshow( dif[yc:yc+of,xc:xc+of] ,cmap="hot", vmin=0, vmax=10)
    inset2.yaxis.set_major_locator(plt.NullLocator())
    inset2.xaxis.set_major_locator(plt.NullLocator())
    insetI = inset_axes(inset2, width="10%",height="100%", loc='lower right', bbox_to_anchor=(0.0, 0., 1, 1),
                     bbox_transform=inset2.transAxes, borderpad=0)
    ...
    fs=16
    axs[0].set_title("Input", fontsize= fs)
    axs[1].set_title("Reconstruction ED path", fontsize= fs)
